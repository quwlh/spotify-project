...
function Browse() {
  const genres = ['Pop', 'Rock', 'Hip Hop', 'Electronic']; // Array of genres
  const subgenres = {
  ...
  }; // Object containing subgenres for each genre

  const [selectedGenreIndex, setSelectedGenreIndex] = useState(0); // State for selected genre index
  const selectedGenre = genres[selectedGenreIndex]; // Selected genre based on index

  const [selectedSubgenres, setSelectedSubgenres] = useState([]); // State for selected subgenres

  const handleLeftGenreClick = () => {
    setSelectedGenreIndex(prevIndex => (prevIndex === 0 ? genres.length - 1 : prevIndex - 1));
    setSelectedSubgenres([]); // Reset selected subgenres when genre changes
  };
